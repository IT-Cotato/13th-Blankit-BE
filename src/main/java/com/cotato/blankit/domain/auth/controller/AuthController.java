...
public class AuthController {
    ...
    @Operation(
            summary = "소셜 로그인",
            description = "소셜 토큰을 검증한 뒤 JWT를 발급합니다. 기존 로그인 세션이 있으면 installationId와 관계없이 만료 처리하고 새 로그인 세션으로 교체합니다."
    )
    @ApiResponses({
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "409", description = "REFRESH_TOKEN_CONFLICT - Refresh Token 처리 충돌")
    })
    @PostMapping("/login")
